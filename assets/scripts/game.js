...
class SimpleGarden {
    constructor() {
        ...
        // SIMPLIFIED WORKING SYSTEM FROM ARCHIVE
        this.cutAreas = []; // Light green mowed areas
        this.growingAreas = []; // Areas watered and growing back to dark
        this.mowerWidth = 80; // Mower size
    ...
    render() {
        ...
        // SIMPLIFIED WORKING SYSTEM - ONLY THE ESSENTIALS
        
        // Draw cut areas (light green for mowed grass)
        this.cutAreas.forEach(cut => {
            this.ctx.fillStyle = '#8BC34A'; // Light green for cut areas
            this.ctx.beginPath();
            this.ctx.arc(cut.x, cut.y, cut.size / 2, 0, 2 * Math.PI);
            this.ctx.fill();
        });
        
        // Draw growing areas (gradually getting darker)
        this.growingAreas.forEach(growing => {
            ...
            const lightR = 139, lightG = 195, lightB = 74; // #8BC34A (light)
            const darkR = 74, darkG = 124, darkB = 42;     // #4A7C2A (dark)
    ...
    setupEvents() {
        document.querySelectorAll('.tool').forEach(tool => {
            ...
            // Add both click and touchstart for reliable tablet interaction
    ...
    addWater(x, y) {
        // EXACT WORKING LOGIC FROM ARCHIVE - SIMPLIFIED
        
        // 1. Add water droplet effect (temporary blue circles)
        ...
        // 2. Check if watering any cut areas - start growing them back
        this.cutAreas.forEach(cutArea => {
            ...
            if (distance < cutArea.size / 2 + 25) {
                ...
                const alreadyGrowing = this.growingAreas.find(growing => 
                    Math.sqrt((growing.x - cutArea.x) ** 2 + (growing.y - cutArea.y) ** 2) < 10
                );
                
                // If not already growing, start it
                if (!alreadyGrowing) {
                    this.growingAreas.push({
                        ...
                        size: cutArea.size,
                        progress: 0,
                        maxProgress: 300 // 5 seconds to grow back to normal
                    });
                    console.log(`💧 Started regrowing cut area at (${cutArea.x}, ${cutArea.y})`);
                }
            }
        });
    ...
    mowArea(x, y) {
        // EXACT WORKING MOWER LOGIC FROM ARCHIVE - SIMPLIFIED
        ...
        // 2. Remove any existing growing areas that get re-mowed
        this.growingAreas = this.growingAreas.filter(growing => {
            const distance = Math.sqrt((growing.x - x) ** 2 + (growing.y - y) ** 2);
            return distance >= this.mowerWidth / 2; // Keep if outside mow area
        });
        
        // 3. Remove grass blades in mowed area
        ...
        // 4. Remove plants that get mowed
        ...
        this.playSound('mow');
        console.log(`🚜 Mowed area at (${x}, ${y}), cut ${beforePlants - this.plants.length} plants, ${beforeBlades - this.grassBlades.length} grass blades`);
    }
    ...
    resetGarden() {
        // SIMPLIFIED RESET - CLEAR ONLY WHAT WE USE
        this.plants = [];
        this.rainbowStrokes = [];
        this.paintStrokes = [];
        this.cutAreas = []; // Mowed areas
        this.growingAreas = []; // Areas growing back
        this.wateredAreas = [];
        this.grassClippings = [];
        this.animals = [];
        this.grassBlades = [];
    ...
    setupPaintControls() {
        ...
        document.querySelectorAll('.brush-circle').forEach(brush => {
            ...
            brush.addEventListener('touchstart', handleBrushClick);
        });
    ...
    update() {
        try {
            ...
            // SIMPLIFIED UPDATE LOGIC - ONLY ESSENTIAL PARTS
            
            // Update growing areas (watered cut areas growing back)
            this.growingAreas.forEach(growing => {
                growing.progress++; // Grow by 1 each frame
                
                // Add grass blades back as it grows - CHECK DENSITY FIRST
                const progress = growing.progress / growing.maxProgress;
                if (progress > 0.5 && Math.random() < 0.02) { // Start later (50%) and slower (2% chance)
                    
                    // Check current density in this area to avoid over-population
                    ...
                    // Expected density based on original createGrass pattern (80% coverage)
                    const areaPixels = Math.PI * (growing.size / 2) * (growing.size / 2);
                    const gridSpacing = 15 * 12; // From createGrass: x += 15, y += 12
                    const expectedBlades = Math.floor((areaPixels / gridSpacing) * 0.8);
                    
                    // Only add if we're below expected density
                    if (existingBlades < expectedBlades * progress) {
                        ...
                        this.grassBlades.push({
                            ...
                            height: 12 + Math.random() * 20, // Closer to original heights
                            swayPhase: Math.random() * Math.PI * 2,
                            swaySpeed: 0.008 + Math.random() * 0.015,
                            swayAmount: 2 + Math.random() * 4
                        });
                ...
                // When fully grown, remove the cut area
                if (growing.progress >= growing.maxProgress) {
                    this.cutAreas = this.cutAreas.filter(cut => {
                        ...
                        return distance > 10; // Remove cut area if close to this growing area
                    });
                }
            });
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('🔥 DOM ready - Initializing splash screen...');
    ...
    const startGame = (e) => {
        ...
        setTimeout(() => {
            ...
            console.log('🌻 Creating SimpleGarden...');
            new SimpleGarden();
        }, 100);
    };
    
    // Use both events but with proper handling
    console.log('🖱️ Adding click listener to splash...');
    splash.addEventListener('click', startGame, { passive: false });
    console.log('👆 Adding touchstart listener to splash...');  
    splash.addEventListener('touchstart', startGame, { passive: false });
    
    // Test if splash is clickable
    splash.addEventListener('click', () => console.log('🔥 SPLASH CLICKED!'), { passive: false });
    splash.addEventListener('touchstart', () => console.log('🔥 SPLASH TOUCHED!'), { passive: false });
    
    console.log('⏰ Setting 4-second auto-start timer...');
